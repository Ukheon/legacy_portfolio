...
class Flame {
	...
	constructor(svg, start, end, width, particle) {
		...
		let colors = ["#F9ECA9", "#EFC900", "#D79700", "#D0BB48"];
	...
	draw() {
		if (this.height > 0) {
			...
			var body = this.particle
				? [
						{ type: "M", points: [0, y] },
						{ type: "L", points: [10, y - heightChunks * 0.2, 20, y - heightChunks * 0.4, 30, y - heightChunks * 0.6] },
				  ]
				: [
						{ type: "M", points: [0, y] },
						{
...
class StageManager {
	...
	addFlame() {
		...
		let start = { x: this.size.width / 2, y: this.size.height - (40 + Math.random() * 20) };
		let end = { x: this.size.width / 4 + Math.random() * (this.size.width / 2), y: particle ? -20 : this.size.height / 3 };
...
let stageManager = new StageManager("svg");
stageManager.init();
...
function drawGrain(holder, path, offset) {
	...
	for (var i = 0; i < chunks.length; i++) {
		...
		var grain = holder.path(pathString).attr({
			fill: "none",
...
function drawStickEnd() {
	...
	stickEnd.ellipse(point[0] + (drawRight ? -3 : 3), point[1], stickThickness / 1.5, stickThickness).attr({
		fill: colors.stick.inside,
	});

	stickEnd.ellipse(point[0] + (drawRight ? -3 : 3), point[1], 2, 3).attr({
